...
func (c *Context) Status() string {
    switch c.status {
        ...
        case errored : return StatusErrored
...
func Log(level uint8, v ...interface { }) {
    switch level {
        case Trace : logger.Trace(v); return
        case Info : logger.Info(v); return
        case Warn : logger.Warn(v); return
        case Error : logger.Error(v); return
    }
}

func Logf(level uint8, msg string, v ...interface { }) {
    switch level {
        case Trace : logger.Tracef(msg, v); return
        case Info : logger.Infof(msg, v); return
        case Warn : logger.Warnf(msg, v); return
        case Error : logger.Errorf(msg, v); return
    }
}
